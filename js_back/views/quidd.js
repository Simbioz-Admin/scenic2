define([
	...
	'text!/templates/panelInfoSource.html'
	],function(_, Backbone, ModelQuidd, SourceTemplate, DestinationTemplate, quiddTemplate, infoTemplate){
		...
		var QuiddView = Backbone.View.extend({
			...
			render : function()
			{
				...
				if(this.options.table == "transfert")
				{
					...
					_.each(shmdatas, function(shmdata, index)
					{
						...
						console.log($(that.el));	
						$("#transfert .sources").prepend($(that.el));
				...
				if(this.options.table == "control")
				{
					var template = _.template(DestinationTemplate, {name : this.model.get("name")});
					$(this.el).html(template);
					$("#control .destinations").append(template);
				}
